...
import { ArrowLeft, MessageSquare, TrendingUp, BookOpen, Users, Clock, Database, Eye, Lightbulb, Shield, Zap, Workflow, BarChart, CheckCircle, Briefcase, CalendarDays, Sparkles } from "lucide-react";
...
const CaseStudyDetail = () => {
  ...
  const study = caseStudies.find(s => 
    s.title.toLowerCase().replace(/[^\w\s]/gi, '').replace(/\s+/g, '-') === slug
  );
    ...
    <main className="min-h-screen bg-transparent pb-20">
      ...
      <div className="container mx-auto px-4 pt-32 pb-16 max-w-4xl">
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-green-100 p-8 md:p-16 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-8">
                {study.paragraphs.map((paragraph, idx) => (
                    <p key={idx} className="text-xl md:text-2xl text-slate-700 leading-relaxed font-medium last:mb-0">
                        {paragraph}
                    </p>
                ))}
            </div>
        ...
        <div className="mt-20 text-center">
            <h3 className="text-2xl font-bold text-slate-900 mb-8">Ready to write your own success story?</h3>
            <Button 
                size="lg" 
                variant="hero" 
