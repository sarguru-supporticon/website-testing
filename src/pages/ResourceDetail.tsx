...
const ResourceDetail = () => {
  ...
  if (!resource) {
    return (
      <main className="container mx-auto px-4 py-3 md:py-16">
        ...
        <p className="mt-4 text-muted-foreground">The resource you are looking for doesn't exist.</p>
        <Link to="/resources" className="text-primary mt-6 inline-block">← Back to Resources</Link>
      </main>
    );
  }
  ...
  const renderContentBlock = (block: ResourceContentBlock, idx: number) => {
    switch (block.type) {
      ...
      case "paragraph":
        return <p key={idx} className="text-base leading-relaxed text-slate-600 mt-3 mb-3 text-left">{block.text}</p>;
      case "list":
        return (
          <ul key={idx} className="list-disc pl-6 my-4 text-slate-600 text-left">
    ...
    <main className="bg-white/30 backdrop-blur-sm min-h-screen pb-20">
      ...
      <article className="container mx-auto px-4 py-3 md:py-16">
        <header className="max-w-4xl mx-auto text-left mb-10">
          <Link to="/resources" className="inline-flex items-center text-sm font-semibold text-green-600 hover:text-green-700 mb-6 group">
            <span className="mr-2 transition-transform group-hover:-translate-x-1">←</span> Back to Resources
          </Link>
          ...
          <h1 className="font-display text-4xl md:text-5xl font-extrabold text-slate-900 leading-tight mb-4 text-left">{resource.title}</h1>
          {resource.subtitle && (
            <p className="text-xl text-slate-500 leading-relaxed text-left">{resource.subtitle}</p>
          )}
        </header>
        ...
        <footer className="max-w-4xl mx-auto mt-12 text-left">
           <div className="inline-flex items-center gap-4 p-6 bg-white/50 backdrop-blur-sm rounded-2xl border border-slate-100">
             <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center shadow-lg">
               <span className="text-white font-bold text-lg">{resource.author?.charAt(0) || "H"}</span>
             </div>
             <div>
               <p className="text-sm font-bold text-slate-800">{resource.author || "Help Dude Team"}</p>
               <p className="text-xs text-slate-500">Expert Insights on Support Engineering</p>
             </div>
           </div>
        </footer>
      </article>
